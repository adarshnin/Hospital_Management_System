$(document).ready(function () {
    ...
    function addProcedure(data) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Procedure Added Successfully", { "status": "success" });
            table.destroy();
    ...
    function updateProcedure(data, code) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "procedure/" + code,
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Procedure Updated Successfully", { "status": "success" });
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getProcedure()
        });
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('.btn-edit').one("click", function (e) {
                ...
                $('#myModal').modal().one('shown.bs.modal', function (e) {
                    ...
                    $("#savetheprocedure").off("click").on("click", function (e) {
                        var instance = $('#detailform').parsley();
                        instance.validate()
                        console.log(instance.isValid())
                        if (instance.isValid()) {
                            jsondata = $('#detailform').serializeJSON();
                            updateProcedure(jsondata, data.code)
                        }
